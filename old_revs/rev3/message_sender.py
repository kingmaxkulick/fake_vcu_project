...
from can_ids import (
    ...
    G_FORCES_ID, BRAKE_TEMP_ID, STATE_CAN_IDS
)
...
def send_state(state):
    state_id = STATE_CAN_IDS[state]
    data = [1]
    message = can.Message(arbitration_id=state_id, data=data, is_extended_id=False)
    try:
        bus.send(message)
        print(f"State '{state}' message sent.")
    except can.CanError as e:
        print(f"Error sending state '{state}': {e}")
